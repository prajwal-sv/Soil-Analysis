...
const ParameterAnalysis = ({ analysis }) => {
  ...
  const renderAnalysisItem = (param, value) => {
    const statusColor = getStatusColor(value);
    
    return (
      <div key={param} className="flex items-center py-2 border-b border-gray-200 last:border-0">
        <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center text-${statusColor}-500 bg-${statusColor}-100`}>
          {getParameterIcon(param)}
        </div>
        <div className="ml-3">
          <p className="text-sm font-medium text-gray-900">{param}</p>
          <p className={`text-sm text-${statusColor}-600`}>{value}</p>
        </div>
      </div>
    );
  };
  ...
  if (!analysis) {
    return (
      <div className="text-center py-4">
        <p className="text-gray-500">No parameter analysis available</p>
      </div>
    );
  }
    ...
    <div className="overflow-hidden">
      <div className="space-y-1">
        {Object.entries(analysis).map(([param, value]) => 
          renderAnalysisItem(param, value)
        )}
      </div>
    </div>
  );
